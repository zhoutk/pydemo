...
def exportMysql(conf, workDir):
    ...
    newDbName = conf["new_db_name"] if "new_db_name" in conf else conf["db_database"]
    for al in rely:
        viewStr = ps[al].replace('`'+conf["db_database"]+'`.', '`'+newDbName+'`.') if "new_db_name" in conf else ps[al]
